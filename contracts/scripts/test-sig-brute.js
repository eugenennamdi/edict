...
async function main() {
  ...
  const messagesToHash = [
    "base" + contractAddress.toLowerCase(),
    "base" + cleanAddr,
    "base0x" + cleanAddr,
    "8453" + contractAddress.toLowerCase(),
    "84532" + contractAddress.toLowerCase(),
    "8453" + cleanAddr,
    "84532" + cleanAddr
  ];
  ...
  for (const msg of messagesToHash) {
    ...
    const sig4 = await deployer.signMessage(msg);
    
    for (const signature of [sig1, sig2, sig3, sig4]) {
      const payload = {
      ...
      if (data.code === "0000") {
        console.log("SUCCESS REGISTER!", msg, signature);
        return;
      }
    }
  }
  console.log("ALL REGISTER FAILED");
}
