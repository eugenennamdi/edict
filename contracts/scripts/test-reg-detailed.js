...
async function main() {
  ...
  const payload1 = { contractAddress, chain, signature: "0x1234" };
  const payload2 = { address: contractAddress, chain, signature: "0x1234" };
  const payload3 = { factoryAddress: contractAddress, chain, signature: "0x1234" };

  for (const p of [payload1, payload2, payload3]) {
      const iv = Buffer.alloc(16, 0);
      const cipher = crypto.createCipheriv('aes-256-cbc', key, iv);
      let encrypted = cipher.update(JSON.stringify(p), 'utf8', 'base64');
      ...
      console.log("Payload keys:", Object.keys(p), "Result:", data);
  }
}
