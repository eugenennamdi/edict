...
import { useStore, AssetType } from "@/lib/store";
...
export function AssetToggle({ 
  ...
  description = "Monitor autonomous compliance policies and real-time vault allocations" 
}: AssetToggleProps) {
  const { activeTab, setActiveTab } = useStore();

  return (
    <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end w-full animate-fade-up gap-4">
      <div>
      ...
      <Tabs 
        value={activeTab} 
        onValueChange={(val) => setActiveTab(val as AssetType)} 
      >
        <TabsList className="grid w-[300px] grid-cols-3 h-11 p-1 bg-black/5 dark:bg-white/[0.05] rounded-full border border-black/5 dark:border-white/[0.05]">
          <TabsTrigger 
            value="USDC" 
            className="rounded-full flex items-center justify-center gap-2 text-[14px] font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-muted-foreground h-full"
          >
            <UsdcIcon className="w-4 h-4 opacity-80" />
            USDC
          </TabsTrigger>
          <TabsTrigger 
            value="ETH" 
            className="rounded-full flex items-center justify-center gap-2 text-[14px] font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-muted-foreground h-full"
          >
            <EthIcon className="w-4 h-4 opacity-80" />
            ETH
          </TabsTrigger>
          <TabsTrigger 
            value="BTC" 
            className="rounded-full flex items-center justify-center gap-2 text-[14px] font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-muted-foreground h-full"
          >
            <BtcIcon className="w-4 h-4 opacity-80" />
            BTC
          </TabsTrigger>
        </TabsList>
      </Tabs>
