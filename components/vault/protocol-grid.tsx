...
import { ProtocolAllocation } from "./protocol-allocation";
      ...
      <CardContent className="p-0">
        <div className="flex flex-col divide-y divide-black/5 dark:divide-white/[0.03]">
          {protocols.map((protocol) => {
            const isRebalancing = protocols.some(p => p.status === 'violation' || p.status === 'rebalancing');
            const hasExited = protocols.some(p => p.status === 'exited');
